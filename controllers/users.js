...
exports.updateUser = (req, res) => {
    ...
    pool.connect((err, client, done) => {
        client.query(
            ...
            [userId, data.first_name, data.last_name, data.email, data.gender, data.is_admin, data.department, data.address],
            (error) => {
                done();
                if (error) {
                    res.status(400).json({
                        status: 'error',
                        error: 'There is an error with your query, please check',
                    });
                } else {
                    res.status(202).send({
                        status: 'success',
...
exports.deleteUser = (req, res) => {
    ...
    pool.connect((err, client, done) => {
        ...
        client.query(query, [userId], (error) => {
            done();
            if (error) {
                res.status(400).json({
                    status: 'error',
